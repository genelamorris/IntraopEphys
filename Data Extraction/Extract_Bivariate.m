function DataStruct = Extract_Bivariate(DataStruct, patient, params)
...
chrparams.tapers = [3 5];
chrparams.Fs = 44000;
chrparams.fpass = [3 60];
...
for hemi = {'right'} % DataStruct.params.hemispheres
    ...
    for row = 1:size(DataStruct.(string(patient)).(TableName),1)
        ...
        for i = 1:numel(Electrodes)
            for j = (i+1):numel(Electrodes)
                ...
                %% %%%%%%%%%%%%%% Spectral Parameters %%%%%%%%%%%%%%%%%
                if params.Compute_PSD
                    % 1. covarience
                    if params.Compute_covarience
                        cov_XY = cov(vec_1_final, vec_2_final); % cov(A,B) is the 2-by-2 covariance matrix
                        DataStruct.(char(patient)).(TableName).Bivariate_Parameters(row).Covariance{i,j} =...
                            cov_XY(1,2);
                    end
                    ...
                    if params.Compute_correlation
                        ...
                        DataStruct.(char(patient)).(TableName).Bivariate_Parameters(row).Corr_coeff{i,j} = ...
                            corr_XY(1,2);
                    end

                    % 3. cross correlation
                    if params.Compute_CrossCorr
                        [cross_corr, lags] = xcorr(vec_1_final,vec_2_final,Fs,'unbiased');
                        DataStruct.(char(patient)).(TableName).Bivariate_Parameters(row).Cross_Corr{i,j} = ...
                            [cross_corr ; lags];
                        DataStruct.(char(patient)).(TableName).Bivariate_Parameters(row).max_corr_lag{i,j} = ...
                            lags(cross_corr == max(cross_corr));
                    end

                    % 4. Cross-PSD
                    if params.Compute_CPSD
                        ...
                        CPSD_cleaned = interp1(f_CPSD(~ismember(1:length(f_CPSD), indices_to_remove)), ...
                            ...
                            f_CPSD);
                        DataStruct.(char(patient)).(TableName).Bivariate_Parameters(row).Cross_PSD{i,j} = ...
                            [CPSD_cleaned ; f_CPSD];
                    end
                    ...
                    if params.Compute_Coherence
                        ...
                        cxy_cleaned = interp1(f_msc(~ismember(1:length(f_msc), indices_to_remove)), ...
                            ...
                            f_msc);
                        DataStruct.(char(patient)).(TableName).Bivariate_Parameters(row).Coherence{i,j} = ...
                            [cxy_cleaned ; f_msc];
                    end

                    % 6. chronux coherencyc
                    if params.Compute_Chronux
                        [C,phi,S12,S1,S2,f_ch] =coherencyc(vec_1,vec_2,chrparams);
                        DataStruct.(char(patient)).(TableName).Bivariate_Parameters(row).Chronux{i,j} = ...
                            {C,phi,S12,S1,S2,f_ch};
                    end

                    % 7. Granger causality
                    if params.Compute_Granger
                        [h_12, p_12] = gctest(double(vec_1)',double(vec_2)');
                        DataStruct.(char(patient)).(TableName).Bivariate_Parameters(row).Granger_test{i,j} = ...
                            [h_12 ; p_12];
                        [h_21, p_21] = gctest(double(vec_2)',double(vec_1)');
                        DataStruct.(char(patient)).(TableName).Bivariate_Parameters(row).Granger_test{j,i} = ...
                            [h_21 ; p_21];
                    end
                end
                ...
                if params.Compute_phase
                    for band = bands
                        ...
                        if params.Save_PhaseDiff
                            DataStruct.(char(patient)).(TableName).Bivariate_Parameters(row).(['Phase_diff_' band.name]){i,j} = phase_diff;
                        end

                        % 2. Phase locking value
                        % also called mean phase coherence / synchrony factor
                        if params.Compute_PLV
                            plv_value = abs(mean(exp(1i * phase_diff)));  % Absolute value of the mean of complex exponential
                            DataStruct.(char(patient)).(TableName).Bivariate_Parameters(row).(['PLV_' band.name]){i,j} = plv_value;
                        end
    ...
    disp(['Finished Loading Patient ' char(patient) ' - ' cell2mat(hemi) ' hemi'])
end
end
